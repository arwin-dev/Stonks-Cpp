#include "dojiRecognizer.h" // Include header file for the dojiRecognizer class

namespace Stonks_Cpp {

	// Constructor for dojiRecognizer class
	dojiRecognizer::dojiRecognizer() : recognizer(1, "Doji")
	{
		// Constructor initializes the dojiRecognizer with a pattern size of 1 and sets the pattern name to "Doji"
	}

	// Override method to recognize a doji pattern for a single candlestick
	bool dojiRecognizer::recognize(List<smartCandlestick^>^ sc, int index)
	{
		// Retrieve the candlestick at the specified index from the list
		smartCandlestick^ sc1 = sc[index];
		bool value;

		// Check if the pattern has already been recognized for this candlestick
		if (sc1->Patterns->TryGetValue(patternName, value))
		{
			// If the pattern has already been recognized, return the stored value
			return value;
		}
		else
		{
			// If the pattern has not been recognized, mark it as a doji and return the result
			sc1->Patterns->Add(patternName, sc1->IsDoji);
			return sc1->IsDoji;
		}
	}
}
